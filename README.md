...
**Rajjo** is a production-grade, local-first autonomous AI desktop agent designed to execute complex, multi-step tasks directly on your computer. Built with Electron, React, FastAPI, LangGraph, dynamic Model Routing, and self-learning episodic and semantic memory, Rajjo plans, executes system tools, learns from past experiences, and provides real-time streaming feedback without exposing raw internal chain-of-thought.
...
- **Unified Model Router**: Switch seamlessly at runtime between Cloud providers (OpenAI, Groq, OpenRouter, Custom base URLs), local Ollama services (with auto-detection), and direct local GGUF models on internal or external drives.
...
- **Safety Boundaries**: Destructive command blocking, configurable execution timeouts, and path safety validation.
- **Dual Memory Layer**:
...
│   ├── config.py             # Centralized paths and secure settings
...
│   │   ├── App.jsx           # Full desktop UI (Chat, Models, Tools, Memory, Settings)
│   │   ├── index.css         # Styling, scrollbars, drag regions
...
```env
# Provider defaults
RAJJO_DEFAULT_PROVIDER=openai
RAJJO_DEFAULT_MODEL=gpt-4o
